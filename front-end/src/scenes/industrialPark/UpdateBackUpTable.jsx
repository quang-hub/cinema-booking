...
import UpdateBackUpDetailTable from "./UpdateBackupDetail";


const UpdateBackUpTable = ({ code }) => {
    ...
    const fetchDataBackup = async (page, pageSize) => {
        try {
            const url = `/vtour/get-all-backup-tour?industrialParkCode=${code}&page=${page + 1}&size=${pageSize}`;
            ...
            setTotalItem(responeData.totalElements || 1);
    ...
    const columns = [
        ...
        {
            field: "tourPath",
            headerName: "Xem chi tiết thay đổi",
            renderCell: (params) => (
                <Box>

                    < IconButton
                        onClick={() => handleClickOpenVr360(params.row.newSceneName)}
                        color="secondary"
                    >
                        <VisibilityIcon />
                    </IconButton >
    ...
    return (
        <Box m="20px">
            ...
            >
                <Header
                    title="Các ảnh đã chỉnh sửa"
                    subtitle="Quản lý ảnh đã chỉnh sửa"
                />
            ...
            {openDialog &&
                <UpdateBackUpDetailTable selectedItem={selectedItem} handleClose={handleCloseDialog}/>
            }
...
export default UpdateBackUpTable;
