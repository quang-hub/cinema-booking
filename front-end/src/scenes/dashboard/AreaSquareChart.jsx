...
const AreaSquare = ({ defaultColors, parkCode }) => {
  ...
  const fetchDataAreaSquare = async () => {
    try {
      const url = `/dash-board/area-by-square`;
      const requestData = {
      ...
      formattedData.push({
        keyCode: "Phần còn thừa",
        value: parseFloat(responseData.data.remaining),
      });

      setDataArea(formattedData);
      const totalValue = formattedData.reduce(
        (acc, area) => acc + parseFloat(area.value),
        0
      );
      setTotal(totalValue);
    } catch (error) {
  ...
  const labelTextColor = isDarkMode ? "text-gray-400" : "text-gray-500";
      ...
      <div className={`${backgroundColor} shadow-lg rounded-lg p-6`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className={`text-lg font-bold ${textColor}`}>
            Thống kê theo diện tích khu đất
          </h2>
        </div>
          ...
          <>
            <div className="w-full h-64 relative flex items-center justify-center">
              ...
              >
                ...
                <span
                  className={`text-lg font-semibold ${labelTextColor}`}
                >
                  Tổng
                </span>
              </div>
